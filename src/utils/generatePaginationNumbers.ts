export const generatePaginationNumbers = (currentPage: number, totalPages: number) => {
  ...
  // mostrar las primeras 3, puntos suspensivo y las ultimas 2
  if (currentPage <= 3) {
    return [1, 2, 3, '...', totalPages - 1, totalPages]
  }

  // si la pagina actual esta entre las ultimas 3
  // montrar las primeras 2 y las ultimas 3
  if (totalPages >= totalPages - 2) {
    return [1, 2, '...', totalPages - 2, totalPages - 1, totalPages]
  }

  // si la pagina actual esta en otro lugar
  // mostrar la primera pagina, puntos suspensivos, la pagina actual y proximos
